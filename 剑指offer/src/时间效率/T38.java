...
//	}
	...
	private static void partitionArray(int[] arr, int low, int hight, int k) {
	...
	private static int partition(int[] arr, int low, int hight) {
		...
		int j = hight;
		while (i < j) {
			while (i < j && arr[hight] >= stard) {
				j--;
			}
			while (i < j && arr[low] <= stard) {
				i++;
			}
			if(i < j) {
			int temp = arr[i];
	...
	public static void main(String[] args) {
		int[] a = new int[] { 1, 2, 5, 4, 5, 3, 8 };
		System.out.println(getLeastNumberss(a, 3));
	}
}
